...
def main():
    parser = argparse.ArgumentParser(
        description="Batch density-map + HTML generation with LWCC."
    )
    parser.add_argument("input_dir", help="Directory containing input images")
    parser.add_argument(
        "-o", "--output_dir", default="output", help="Where to drop maps & HTML"
    )
    parser.add_argument(
        "--model_name", choices=VALID_MODELS, default="DM-Count", help="Model to use"
    )
    parser.add_argument(
        "--model_weights", choices=VALID_WEIGHTS, default="SHA", help="Weights to use"
    )
    args = parser.parse_args()

    os.makedirs(args.output_dir, exist_ok=True)
    image_files = [
        p for p in glob.glob(os.path.join(args.input_dir, "*")) if is_image_file(p)
    ]
    results = []
    ...
    for image_path in image_files:
        ...
        count = generate_density_map(
            dest_orig, density_path, args.model_name, args.model_weights
        )
    ...
    with open(html_path, "w") as f:
        ...
        for orig, dens, cnt in results:
            ...
            f.write(
                f"  <img src='{orig}' alt='original' style='width:45%; margin-right:5%;'>\n"
            )
